...
char BMP085::bmp085Read(byte address)
{
  byte result;
  readRegister(address, 1, &result);
  return (char)result;
}
...
int BMP085::readRegister(byte reg_addr, int nbytes, byte *buffer)
{
	...
	Wire.requestFrom(BMP085_ADDRESS, nbytes);
...
int BMP085::bmp085ReadInt(byte address)
{
  byte buffer[2];
  byte adr = 0xFC;
  readRegister(address, 2, buffer);
